...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        //final dimensions with border added
        int wBorder = w + (2 * borderWidth);
        int hBorder = h + (2 * borderWidth);
        
    	Pixel [][] src = pic.getBitmap();
    	Pixel [][] tgt = new Pixel[hBorder][wBorder];
    	
    	//vertical borders
    	for (int col = 0; col < h; col++) {
    		for (int row = 0; row < borderWidth; row++) {
    			tgt[col + borderWidth][row] = borderColor;
    			tgt[col + borderWidth][wBorder - row - 1] = borderColor;
         	}
         }
    	
    	//hoizontal borders
    	for (int col = 0; col < borderWidth; col++) {
    		for (int row = 0; row < wBorder; row++) {
    			tgt[col][row] = borderColor;
    			tgt [hBorder - col - 1][row] = borderColor;
         	}
         }
    ...
    public static PixelPicture alphaBlend(
        ...
        if (w == fW && h == fH) {
            for (int col = 0; col < w; col++) {
                for (int row = 0; row < h; row++) {
                	Pixel p1 = bmpPic[row][col];
                	Pixel p2 = bmpF[row][col];
                    ...
                    bmpPic[row][col] = new Pixel(r, g, b);
                }
            }
        }
        //returns first pic if different dimensions but if same, bmpPic is transformed
        return new PixelPicture(bmpPic); 
    }
